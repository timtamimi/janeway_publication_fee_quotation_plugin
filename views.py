...
from plugins.publication_fee_quotation import forms, logic, models
...
def manager(request):
    ...
    template = "publication_fee_quotation/manager.html"
    context = {
        "form": form,
        "config": config,
...
def webhook_callback(request, journal_code):
    ...
    if not quote_id:
        logger.warning(f"Missing quote_id in webhook payload for journal: {journal_code}")
    ...
    try:
        quotation = models.FeeQuotation.objects.filter(
...
def quotation_detail(request, quotation_id):
    ...
    template = "publication_fee_quotation/quotation_detail.html"
    context = {
        "quotation": quotation,
    }
