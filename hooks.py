...
from plugins.janeway_publication_fee_quotation_plugin import models, logic
...
def inject_fee_quotation_ui(context):
    ...
    if not logic.is_quotation_required_for_article(article):
        logger.debug(
            f"Fee quotation not required for article {article.pk} (section: {article.section})"
        )
        return ""
    ...
    if quotation and should_void_quotation(article, quotation):
        logic.void_article_quotations(
            article, reason="Article was modified after quotation was requested."
        )
    ...
    try:
        return render_to_string(
            "janeway_publication_fee_quotation_plugin/elements/submission_hook.html",
            hook_context,
            request=request,
        )
