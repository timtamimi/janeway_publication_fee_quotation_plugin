...
from plugins.publication_fee_quotation import models
    ...
    if affiliation and affiliation.organization:
        ...
        if location:
            ...
            if location.country:
                ...
                address["country"] = location.country.code.lower() if location.country.code else ""
...
def get_or_create_quotation(article, author):
    ...
    existing = models.FeeQuotation.objects.filter(
        article=article,
        author=author,
        status__in=[
...
def request_fee_quotation(article, author, request):
    ...
    if config.api_headers:
        try:
            ...
            logger.warning(f"Invalid API headers JSON for journal {article.journal.code}")
    
    # Make the API request
    try:
        logger.info(f"Requesting fee quotation from {config.api_url} for article {article.pk}")
        ...
        if config.quotation_url_template:
            ...
            quotation_url = (
                get_nested_value(response_data, "url") or
                get_nested_value(response_data, "quotation_url") or
                get_nested_value(response_data, "redirect_url") or
                get_nested_value(response_data, "quote_url")
            )
            if quotation_url:
                quotation.quotation_url = quotation_url
            else:
        ...
        quotation.save()
        logger.info(f"Fee quotation {quotation.pk} presented with URL: {quotation.quotation_url}")
        ...
        logger.error(f"Invalid JSON response for fee quotation for article {article.pk}")
